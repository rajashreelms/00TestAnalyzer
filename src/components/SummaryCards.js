...
const cards = [
  ...
  { key: 'active', label: 'Active Employees', icon: PersonIcon, color: '#1a73e8',
    tooltip: 'Employees with non-zero /559 (Transfer to bank) in current period' },
  { key: 'zero', label: 'Zero Pay', icon: WarningAmberIcon, color: '#e65100', bg: '#fff8e1',
    tooltip: 'Employees whose /559 (Transfer to bank) is zero in current period — may be on leave, suspended, or pending adjustment' },
  ...
  { key: 'disc', label: 'Discrepancies', icon: ReportProblemIcon, color: '#e65100', bg: '#fff8e1',
    tooltip: 'Count of /559 variances exceeding the threshold — these employees need review' },
  { key: 'critical', label: 'Critical (>10%)', icon: ErrorIcon, color: '#c62828', bg: '#fce8e8',
    tooltip: 'Employees where /559 variance exceeds 10% — high priority for review' },
  { key: 'avgVar', label: 'Avg Variance', icon: TrendingUpIcon, color: '#1a73e8',
    tooltip: 'Average absolute /559 variance % across all active employees' },
  { key: 'recon', label: 'Perfect Recon', icon: CheckCircleIcon, color: '#1e8e3e', bg: '#e6f4ea',
    tooltip: 'Employees where Sum of (WT × Multiplier) matches /559 within ±1 in both periods — bifurcation fully explains net pay' },
];

export default function SummaryCards({ active, zeroPay, removed, added, detailed }) {
